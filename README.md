<h1 align="center">Jiho Chung</h1>

<p align="center">
  <strong>Undergraduate AI Researcher @ <a href="https://sites.google.com/view/aimhi-lab">AIMHI Lab</a>, Gachon University</strong><br />
  Explainable AI for Medical Imaging
</p>

<p align="center">
  <a href="https://sites.google.com/view/aimhi-lab"><img src="https://img.shields.io/badge/AIMHI_Lab-0A4D68?style=for-the-badge&logo=googlechrome&logoColor=white" alt="AIMHI Lab" /></a>
  <a href="https://jhch1113.github.io/"><img src="https://img.shields.io/badge/Homepage-114B5F?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Homepage" /></a>
  <a href="https://www.acmicpc.net/user/mtn2072"><img src="https://img.shields.io/badge/BOJ-1F6FEB?style=for-the-badge&logo=codeforces&logoColor=white" alt="BOJ" /></a>
  <a href="https://solved.ac/mtn2072"><img src="https://img.shields.io/badge/Solved.ac-0E7490?style=for-the-badge&logo=bookstack&logoColor=white" alt="Solved.ac" /></a>
</p>

---

## Profile Snapshot

- Affiliation: AIMHI Lab, Gachon University
- Main Interests: Explainable AI, Medical Image Understanding, Vision-Language Models
- Current Focus: Building an auditable multimodal bottleneck system for medical reporting

## Research Interests

- Explainable AI for Medical Imaging: making model decisions transparent and clinically interpretable.
- Language-Aligned Visual Prototypes: aligning visual evidence with medical language concepts.
- Hallucination-Controlled Medical VLMs: generating reliable, structured reports with constrained decoding.
- Robust Multimodal Learning: improving stability under scanner, protocol, and domain shifts.

## Contact and Links

- Email: mtn2072@gachon.ac.kr

---
